...
module.exports = {
  convert: function(record){
    ...
    if(record['UT']){
        g.add(env.createTriple(uri, 'owl:sameAs', 'urn:'+record['UT'][0]));
    }
    if(record['DI']){
    ...
    if(record['AF']){
      ...
      _.forEach(record['AF'], function(author,k) {
        ...
        g.add(env.createTriple(uri, 'wosV:AF', 'wosR:'+author_id));
        g.add(env.createTriple('wosR:'+author_id, 'rdfs:label', author.l()));
        //add skos lat label for short author names
        if(record['AU'] && record['AU'][k]){
          g.add(env.createTriple('wosR:'+author_id, 'skos:altLabel', record['AU'][k].l()));
        }
      });
    }
    if(record['WC']){
